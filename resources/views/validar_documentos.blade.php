		...
		<div class="container d-flex justify-content-between" style="margin-left: -10px;">
	     ...
	     <input type="hidden" name="_token" value="{{ csrf_token() }}">
          <table class="table table-sm table-bordered" style="font-size: 12px;">
            <?php $qtdValidar = sizeof($documentos); $qtdApr = sizeof($aprovacao); ?>
              @if($qtdApr > 0) 
              ...
              <td><center>SELECIONAR <br><input onclick="habilitar('sim')" type="checkbox" id="checkAll1" name="checkAll1" /></center> </td>
                ...
                <td ><center>VISUALIZAR</center></td>
				 ...
				 <td> <br><center> <b> @if($docs->unidade_id == 1) <?php echo "HCP GESTÃO"; ?>  
                      @elseif($docs->unidade_id == 2) <?php echo "HMR"; ?>  
                      @elseif($docs->unidade_id == 3) <?php echo "BELO JARDIM"; ?>  
                      @elseif($docs->unidade_id == 4) <?php echo "ARCOVERDE"; ?>  
                      @elseif($docs->unidade_id == 5) <?php echo "ARRUDA"; ?>  
                      @elseif($docs->unidade_id == 6) <?php echo "CARUARU"; ?>  
                      @elseif($docs->unidade_id == 7) <?php echo "HSS"; ?>  
                      @elseif($docs->unidade_id == 8) <?php echo "HPR"; ?>  
                      @endif </b> </center> </td>
                 <td title="<?php echo $docs->nome; ?>"> <p><center>  {{ $docs->nome }} </center></p></td> 
                 <td title="<?php echo $docs->numeroDoc; ?>"> <p><center> {{ $docs->numeroDoc }} </center> </p>  </td>
                 <td> <?php $qtdA = sizeof($aprovacoes); ?>
                 <center>
                 <?php for($ap = 0; $ap < $qtdA; $ap++) { ?>
                 @if($aprovacoes[$ap]->documento_id == $docs->id)
                 ...
                 <td><br>
                 <center>
                 <button type="button" class="btn btn-info btn-sm" data-toggle="modal" data-target="#exampleModal<?php echo $docs->id ?>"> 
                    Observação
                  </button> 
                  <div class="modal fade" id="exampleModal<?php echo $docs->id ?>" >
                  ...
                  <div class='modal-content'>
                    ...
                    <div class='panel-body'>
                      <p align="justify">{{ $docs->observacao }}</a>
                    </div>
                    </div>
                  </div>
                  </div>
                 ...
                 <td> <p><center> <a href="{{ route('validar_fluxo', $docs->id) }}" target="_blank" class="btn btn-dark btn-sm">Visualizar</a> </center></p></td>
                 </td> 
                 <?php $a += 1; ?>
                @endforeach
               @endif
